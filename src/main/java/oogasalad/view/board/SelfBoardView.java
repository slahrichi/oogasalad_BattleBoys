  ...
  public void initializeCellViews(CellState[][] arrayLayout) {
    for (int row = 0; row < arrayLayout.length; row++) {
      for (int col = 0; col < arrayLayout[0].length; col++) {
        ...
        CellView cell = new CellView(points, arrayLayout[row][col] == CellState.NOT_DEFINED ? Color.valueOf(myCellStateResources.getString(FILL_PREFIX+CellState.NOT_DEFINED.name())) : arrayLayout[row][col] == CellState.SHIP_HEALTHY ? Color.valueOf(
